...
DOC_PATH = PROJECT_ROOT / "docs" / "quant_score_contribution_2026-05-28.md"
...
def build_report() -> dict[str, object]:
    ...
    for column in ["selection_score", "quant_score"]:
        snapshots[column] = pd.to_numeric(snapshots.get(column), errors="coerce").fillna(50.0)
    snapshots["selection_plus_quant_tilt"] = (
        snapshots["selection_score"] + (snapshots["quant_score"] - 50.0) * 0.16
    ...
    variants = [
        ...
        ("selection_score", snapshots.loc[snapshots["quant_score"].ge(58.0)].copy(), 3, "selection_quant_ge_58"),
        ("selection_score", snapshots.loc[snapshots["quant_score"].ge(68.0)].copy(), 3, "selection_quant_ge_68"),
        ("selection_score", snapshots.loc[snapshots["quant_score"].le(45.0)].copy(), 3, "selection_quant_le_45"),
        ("selection_plus_quant_tilt", snapshots.copy(), 3, "selection_plus_quant_tilt"),
        ("quant_score", snapshots.copy(), 3, "quant_only_top3"),
    ]
        ...
        summary_rows.append(
            {
                ...
                "avg_quant_score": float(pd.to_numeric(candidates.get("quant_score"), errors="coerce").mean()) if not candidates.empty else 0.0,
                **result.summary,
            }
        )
    ...
    best_gate_row = summary_df.loc[summary_df["variant"].isin(["selection_quant_ge_58", "selection_quant_ge_68", "selection_quant_le_45"])].sort_values(
        ["annualized_return", "ending_equity"], ascending=[False, False]
    ).iloc[0]
    tilt_row = summary_df.loc[summary_df["variant"].eq("selection_plus_quant_tilt")].iloc[0]
    quant_only_row = summary_df.loc[summary_df["variant"].eq("quant_only_top3")].iloc[0]
    ...
    payload = {
        ...
        "findings": [
            ...
            f"The best quant gate is `{best_gate_row['variant']}` at {float(best_gate_row['annualized_return']):.2%} annualized.",
            f"`quant_only_top3` lands at {float(quant_only_row['annualized_return']):.2%} annualized, which shows quant is not a strong standalone ranker on this slice.",
            f"`selection_plus_quant_tilt` lands at {float(tilt_row['annualized_return']):.2%} annualized, so overweighting quant on top of selection does not beat the current baseline on this slice.",
        ],
    }

    summary_csv = OUTPUT_DIR / "quant_score_contribution_summary.csv"
    json_path = OUTPUT_DIR / "quant_score_contribution.json"
